...
**Script**: `scripts/maintenance/precompute-topic-clusters.ts`
...
**IMPORTANT**: TopicsView supports dual granularity modes with separate clustering for each.

### Keyword Graph (Clustering Input)
- **Endpoint**: `/api/topics?nodeType=article|chunk`
- **Query**: `getKeywordBackbone` → `get_keyword_graph(filter_node_type)` RPC
- **Filter**: `node_type` parameter (article or chunk keywords)
- **Purpose**: Main graph visualization and clustering
- **Examples**:
  - Article mode: "machine learning", "neural networks", "deep learning"
  - Chunk mode: More granular keywords from individual paragraphs

### LOD Nodes (Background Layer)
- **Endpoint**: `/api/topics/chunks`
- **Query**: Direct Supabase join on `keywords` and `nodes` tables
- **Filter**: `nodes.node_type` matches current granularity mode
- **Purpose**: Text content behind frosted glass panel
- **Symmetric Design**:
  - Article mode: Shows article content (title + summary)
  - Chunk mode: Shows chunk content (paragraph text)

**Why This Matters**: The `nodeType` parameter controls both the keyword graph AND the LOD nodes, ensuring they match semantically. Switching modes triggers cache invalidation to prevent showing mismatched content.
...
- Regenerate: `npm run script scripts/maintenance/precompute-topic-clusters.ts`
...
- `scripts/maintenance/precompute-topic-clusters.ts` - Batch generation
