...
describe("applyFisheyeCompression", () => {
  // Use same values for width/height to maintain circular behavior for backward-compatible tests
  ...
  it("does not compress keywords within compression start radius", () => {
    ...
    const result = applyFisheyeCompression(
      250, 0, camX, camY,
      compressionStartRadius, compressionStartRadius,
      maxRadius, maxRadius
    );
  ...
  it("preserves direction while compressing distance", () => {
    ...
    // With Lp norm (p=6), diagonal directions have larger horizon distance than pure circular
    // So we just verify compression happened, not that it's within circular maxRadius
  });
  ...
  it("handles negative coordinates correctly", () => {
    ...
    // Verify compression happened (distance reduced)
    expect(distance).toBeLessThan(originalDistance);
  });
});
