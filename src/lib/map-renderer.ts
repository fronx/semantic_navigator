...
export interface HighlightConfig {
  ...
  baseDim: number;
  /** Function to compute link opacity based on similarity */
...
export interface MapRenderer {
  ...
  /** Apply highlight/dim effect to nodes and links (renderer-agnostic API) */
  applyHighlight: (config: HighlightConfig) => void;
  /** Get node selection for external styling (search highlighting) - D3 specific */
  nodeSelection: d3.Selection<SVGGElement, SimNode, SVGGElement, unknown>;
  /** Get link selection for external styling - D3 specific */
...
export function createRenderer(options: RendererOptions): MapRenderer {
  ...
  // This is the renderer-agnostic API for hover highlighting
  function applyHighlight(config: HighlightConfig) {
    const { highlightedIds, baseDim, computeLinkOpacity } = config;

    if (highlightedIds === null) {
      // Dim everything slightly (nothing nearby)
      const dim = 1 - baseDim;
      nodeSelection.select("circle").attr("opacity", dim);
      linkSelection.attr("stroke-opacity", (d) => computeLinkOpacity(d.similarity ?? 0.5) * dim);
    } else if (highlightedIds.size === 0) {
      // Restore full opacity (hover ended)
      nodeSelection.select("circle").attr("opacity", 1);
      linkSelection.attr("stroke-opacity", (d) => computeLinkOpacity(d.similarity ?? 0.5));
    } else {
      ...
      linkSelection.attr("stroke-opacity", (d) => {
        ...
        return bothHighlighted ? computeLinkOpacity(d.similarity ?? 0.5) : 0.05;
      });
    }
  }
