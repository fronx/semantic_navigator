...
export interface TopicsViewProps {
  ...
  /** Renderer type: "d3" (default) or "three" for WebGL */
...
export function TopicsView({
  ...
  // Store renderer for cluster update effect (unified type)
  const rendererRef = useRef<(MapRenderer | ThreeRenderer) | null>(null);

  // Main rendering effect for D3
  useEffect(() => {
    ...
    const computeEdgeOpacity = (similarity: number) => {
      const adjusted = applyContrast(similarity, contrast);
      return 0.1 + adjusted * 0.7;
    };

    // Apply per-edge opacity based on similarity with contrast
    renderer.linkSelection.attr("stroke-opacity", (d) => computeEdgeOpacity(d.similarity ?? 0.5));
    ...
    // Hover highlight helper using renderer-agnostic API
    function handleHighlight(highlightedIds: Set<string> | null) {
      const { baseDim } = hoverConfigRef.current;
      renderer.applyHighlight({
        highlightedIds,
        baseDim,
        computeLinkOpacity: computeEdgeOpacity,
      });
    }
    ...
    d3.select(svg)
      .on("mousemove.hover", (event: MouseEvent) => {
        ...
        if (spatialIds.size === 0) {
          ...
          handleHighlight(highlightedIds);
        }
      })
      .on("mouseleave.hover", () => {
        handleHighlight(new Set());
      });
  ...
  }, [rendererType, keywordNodes, edges, knnStrength, contrast, hoverConfig.screenRadiusFraction]);

  // Main rendering effect for Three.js
  useEffect(() => {
    ...
    // Store nodes for cluster updates
    ...
    const renderer = createThreeRenderer({
      container,
      nodes: mapNodes as SimNode[],
      links: mapEdges as SimLink[],
      immediateParams,
      callbacks: {
        onKeywordClick: handleKeywordClick,
        onZoomEnd: (transform) => {
    ...
    // Set up hover highlighting via vanilla JS events
    const screenRadiusFraction = hoverConfig.screenRadiusFraction ?? DEFAULT_HOVER_CONFIG.screenRadiusFraction!;
    const adjacency = buildAdjacencyMap(mapEdges);
    const embeddings = buildEmbeddingMap(keywordNodes, (n) => `kw:${n.label}`);
    ...
    function onMouseMove(event: MouseEvent) {
      ...
      if (result.spatialIds.size === 0) {
        ...
        handleHighlight(result.highlightedIds);
      }
    }
  ...
  }, [rendererType, keywordNodes, edges, contrast, hoverConfig.screenRadiusFraction, handleKeywordClick, handleZoomChange, hoverConfigRef]);
